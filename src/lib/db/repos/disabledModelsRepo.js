...
export async function disableModels(providerAlias, ids) {
  ...
  await db.transaction(async (txn) => {
    const row = await txn.get(`SELECT value FROM kv WHERE scope = ? AND key = ?`, [SCOPE, providerAlias]);
...
export async function enableModels(providerAlias, ids) {
  ...
  await db.transaction(async (txn) => {
    if (!Array.isArray(ids) || ids.length === 0) {
      await txn.run(`DELETE FROM kv WHERE scope = ? AND key = ?`, [SCOPE, providerAlias]);
      return;
    }
    const row = await txn.get(`SELECT value FROM kv WHERE scope = ? AND key = ?`, [SCOPE, providerAlias]);
    const current = row ? (parseJson(row.value, []) || []) : [];
