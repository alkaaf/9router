...
export {
  ...
  saveRequestUsage, flushWriteQueue, getUsageHistory, getUsageStats, getChartData,
...
export async function exportDb() {
  ...
  const out = {
    ...
    providerConnections: (await db.all(`SELECT * FROM providerConnections`)).map((r) => ({ ...parseJson(r.data, {}), id: r.id, provider: r.provider, authType: r.authType || "oauth", name: r.name, email: r.email, priority: r.priority, isActive: r.isActive === 1 || r.isActive === true, createdAt: r.createdAt, updatedAt: r.updatedAt })),
...
export async function importDb(payload) {
  ...
  await db.transaction(async (txn) => {
    await txn.run(`DELETE FROM settings`);
    await txn.run(`DELETE FROM providerConnections`);
    await txn.run(`DELETE FROM providerNodes`);
    await txn.run(`DELETE FROM proxyPools`);
    await txn.run(`DELETE FROM apiKeys`);
    await txn.run(`DELETE FROM combos`);
    await txn.run(`DELETE FROM kv WHERE scope IN ('modelAliases', 'customModels', 'mitmAlias', 'pricing')`);

    if (payload.settings) {
      await txn.run(`INSERT INTO settings(id, data) VALUES(1, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data`, [stringifyJson(payload.settings)]);
    }
    ...
    for (const [a, m] of Object.entries(payload.modelAliases || {})) {
      await txn.run(`INSERT INTO kv(scope, key, value) VALUES('modelAliases', ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value=excluded.value`, [a, stringifyJson(m)]);
    }
    for (const m of payload.customModels || []) {
      const k = `${m.providerAlias}|${m.id}|${m.type || "llm"}`;
      await txn.run(`INSERT INTO kv(scope, key, value) VALUES('customModels', ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value=excluded.value`, [k, stringifyJson(m)]);
    }
    for (const [tool, mappings] of Object.entries(payload.mitmAlias || {})) {
      await txn.run(`INSERT INTO kv(scope, key, value) VALUES('mitmAlias', ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value=excluded.value`, [tool, stringifyJson(mappings || {})]);
    }
    for (const [provider, models] of Object.entries(payload.pricing || {})) {
      await txn.run(`INSERT INTO kv(scope, key, value) VALUES('pricing', ?, ?) ON CONFLICT(scope, key) DO UPDATE SET value=excluded.value`, [provider, stringifyJson(models || {})]);
    }
  });
